...
01-utils-wiring은 `CONDITIONAL PASS` 상태이며, 02에서는 01 유틸 정책을 재설계하지 않는다.
이번 단계의 핵심은 mock 데이터 삭제가 아니라 **mock/real 경계 분리**다.

다음 원칙을 반드시 지킨다.

1. real 모드에서 API 실패를 mock 결과로 대체하지 않는다.
2. real 모드에서 `riskScore`만으로 `frame_scores`, `suspicious_segments`, `detection_reasons`, `reasonGroups`를 생성하지 않는다.
3. `riskScore`가 백엔드에서 온 실제 값이면 전체 요약 점수로만 표시할 수 있다.
4. `COMPLETED` 상태인데 근거 데이터가 없으면 "분석 근거 없음" 상태를 표시한다.
5. "분석 근거 없음"을 "정상", "원본", "딥페이크 아님"처럼 표현하지 않는다.
6. "AI 연동 전"은 일반 사용자 화면에 그대로 노출하지 않고, 필요하면 "현재 AI 분석 결과를 사용할 수 없습니다"처럼 표현한다.
7. unknown status를 success 또는 completed로 간주하지 않는다.
8. mock 생성기는 `lib/mock` 또는 route-local `_mock` 아래에만 둔다.
9. 화면 컴포넌트에서 mock 생성기를 직접 import하지 않게 한다.
10. mock 분기는 가능하면 API 모듈 레이어에만 둔다.
11. `components/analysis-result.tsx`가 데모용 sample builder를 계속 가져야 한다면, 해당 함수는 mock 전용 파일로 이동하거나 mock 전용 prop 경로에서만 사용되게 한다.
12. mock 데이터가 다운로드, 보고서, 증거 내보내기 경로에 포함되지 않게 확인한다.
13. `localStorage`, `sessionStorage`, query cache, persist store에 남은 mock 데이터가 real 모드에 노출되지 않는지 확인한다.
14. 02 범위 밖인 컴포넌트 대규모 분리, 디자인 개편, 상태관리 교체는 하지 않는다.
15. lint/typecheck 실행 불가 상태는 해결하려 하지 않고 리포트에 그대로 남긴다.
16. `next-env.d.ts`, `tsconfig.tsbuildinfo`, `docs/frontend/local-dev.md`, `tmp-upload-test.mp4`는 기존 변경물이므로 되돌리거나 정리하지 않는다.
...
완료 후 `docs/frontend/refactor/reports/02-mock-isolation.report.md`에 다음을 반드시 작성한다.
